...
import org.apache.dubbo.spring.boot.autoconfigure.observability.zipkin.customizer.Zipkin4RestTemplateBuilderCustomizer;
import org.apache.dubbo.spring.boot.autoconfigure.observability.zipkin.customizer.ZipkinWebClientBuilderCustomizer;
...
/**
 * Configurations for Zipkin on Spring Boot 4. Those are imported by {@link DubboZipkin4AutoConfiguration}.
 */
class Zipkin4Configurations {
    ...
    static class RestTemplateSenderConfiguration {
        ...
        @Bean
        @ConditionalOnMissingBean(Sender.class)
        Zipkin4RestTemplateSender restTemplateSender(
                DubboConfigurationProperties properties,
                ObjectProvider<Zipkin4RestTemplateBuilderCustomizer> customizers) {
            ExporterConfig.ZipkinConfig zipkinConfig =
                    properties.getTracing().getTracingExporter().getZipkinConfig();
            RestTemplateBuilder restTemplateBuilder = new RestTemplateBuilder()
                    .connectTimeout(zipkinConfig.getConnectTimeout())
                    .readTimeout(zipkinConfig.getReadTimeout());
            restTemplateBuilder = applyCustomizers(restTemplateBuilder, customizers);
            return new Zipkin4RestTemplateSender(zipkinConfig.getEndpoint(), restTemplateBuilder.build());
        }

        private RestTemplateBuilder applyCustomizers(
                RestTemplateBuilder restTemplateBuilder,
                ObjectProvider<Zipkin4RestTemplateBuilderCustomizer> customizers) {
            Iterable<Zipkin4RestTemplateBuilderCustomizer> orderedCustomizers =
                    () -> customizers.orderedStream().iterator();
            RestTemplateBuilder currentBuilder = restTemplateBuilder;
            for (Zipkin4RestTemplateBuilderCustomizer customizer : orderedCustomizers) {
